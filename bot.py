...
PROFILE_TEXT = "👤 Пр��филь\n\n🏦 Баланс: {balance} {uah}\n🛍️ Заказов: {orders}"
HELP_TEXT = "Вопросы? Обратись: @italiansquare"
TOPUP_TEXT = f"💳 Введи сумму пополнения (мин. 10 {UAH}):"

# Описания товаров
PRODUCT_DESCRIPTIONS = {
    "Ш": "Премиум качество. Легальный магазин ссылок на книги. Быстрая доставка по почте.",
    "ХаН": "Отличный выбор. Легальный магазин ссылок на книги. Шустрые отправки."
}
...
async def cb_product(call: CallbackQuery):
    ...
    # Получаем первый товар для получения фото
    first_item = rows[0]
    price = decimal.Decimal(first_item["price"])
    
    # Полное описание из словаря
    full_desc = PRODUCT_DESCRIPTIONS.get(product, first_item["description"] or "Без описания")
    
    text = f"✅ {product}\n💰 Цена: {price:.0f} {UAH}\n\n{full_desc}\n\n📍 Выбери район:"
    
    if first_item["photo_id"]:
        await call.message.answer_photo(
            photo=first_item["photo_id"],
            caption=text,
...
async def cb_district(call: CallbackQuery):
    ...
    full_desc = PRODUCT_DESCRIPTIONS.get(product, item["description"] or "Без описания")
